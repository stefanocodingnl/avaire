...
public class WatchRoleCommand extends Command {
    ...
    public WatchRoleCommand(AvaIre avaire) {
        super(avaire, false);
    }

    @Override
    public String getName() {
        return "Watch Role Command";
    }
    ...
    public List<String> getTriggers() {
        return Collections.singletonList("watchrole");
    }
    ...
    private boolean sendMutedRole(CommandMessage context, GuildTransformer guildTransformer) {
        ...
        Role role = context.getGuild().getRoleById(guildTransformer.getOnWatchRole());
        if (role == null) {
            context.makeInfo(context.i18n("noMuteRoleSet"))
                .set("command", generateCommandTrigger(context.getMessage()))
                .queue();
