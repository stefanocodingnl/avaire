    ...
    private boolean handleShow(CommandMessage context, String[] args) {
        ...
        try {
            ...
            context.makeInfo(guild.getId())
                ...
                .addField("Owner", guild.retrieveOwner().submit().getNow(null).getUser().getName() + "#" + guild.retrieveOwner().submit().getNow(null).getUser().getDiscriminator(), true)
                .addField("Owner ID", guild.retrieveOwner().submit().getNow(null).getUser().getId(), true)
